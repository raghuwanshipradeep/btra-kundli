...
async def claim(order_id: str, attempts: int) -> bool:
    """Mark every row sharing this order_id 'generating' and bump the attempt counter.

    Updating all duplicate rows (a sheet edit can insert a second row with the same order_id)
    keeps their status consistent and is what makes a later 'archived' cover them too.
    """
    return await _update(
        order_id,
        {"kundli_status": "generating", "kundli_attempts": attempts + 1},
        log="claim",
    )
