...
async def sweep_once(limit: int = 50) -> dict:
    ...
    async with _SHEET_WORKER_LOCK:
        # Whose credits fund this sweep. None = gating disabled (unmetered, pre-credit
        # behaviour). Resolved once per sweep; the balance itself is re-read per order
        # because it moves as we spend and an admin can top up at any time.
        astrologer_id = await credits_repo.resolve_astrologer_id()
        if credits_repo.enabled() and not astrologer_id:
            # Fail closed: gating is on but we can't tell whose balance to charge.
            logger.error("SHEET NO ASTROLOGER: no active astrologer resolved; skipping sweep")
            summary["status"] = "no_astrologer"
            return summary
        ...
        for row in rows:
            ...
            # Credit gate, deliberately BEFORE the claim: a blocked order must cost nothing.
            # Not claiming leaves kundli_status null and kundli_attempts untouched, so the row
            # is picked up and generated by itself once credits are topped up — no operator
            # action, no burnt attempt, no API spend. balance None = unknown (Supabase error),
            # 0 = genuinely empty; both block, and the log says which.
            if astrologer_id:
                balance = await credits_repo.get_balance(astrologer_id)
                if balance is None or balance <= 0:
                    remaining = len(rows) - (
                        summary["processed"] + summary["skipped"] + summary["failed"]
                    )
                    logger.error(
                        "SHEET NO CREDITS: balance=%s — stopping sweep; %d order(s) left pending, "
                        "they will generate automatically once credits are added",
                        balance, remaining,
                    )
                    summary["status"] = "no_credits"
                    summary["credits_balance"] = balance
                    summary["pending_blocked"] = remaining
                    break
            ...
            try:
                ...
                # Spend the credit here — the PDF is in Drive, so value is delivered. Placed
                # before mark_done on purpose: if mark_done fails the file still exists and the
                # credit was genuinely consumed, so it must not be refunded.
                if astrologer_id:
                    spent, balance_after = await credits_repo.consume_credit(astrologer_id, order_id)
                    if spent:
                        logger.info(
                            "SHEET CREDIT USED: order=%s balance_after=%s", order_id, balance_after,
                        )
                    else:
                        # The only case where a PDF exists with no matching ledger row: the
                        # balance raced to 0 via the admin app, or the RPC is unavailable.
                        # Never fail the order over it — just make it impossible to miss.
                        logger.error(
                            "SHEET CREDIT NOT DEDUCTED: order=%s balance=%s — PDF delivered but "
                            "ledger not updated; reconcile manually",
                            order_id, balance_after,
                        )
                        summary.setdefault("credit_warnings", []).append(order_id)
