...
def _has_sadhesati_data(data: KundliData) -> bool:
    if data.sadhesati_life_details:
        ...
        if isinstance(data.sadhesati_life_details, dict) and data.sadhesati_life_details:
            return True
    if data.sadhesati_current_status:
        if isinstance(data.sadhesati_current_status, dict):
            is_undergoing = data.sadhesati_current_status.get("is_undergoing") or data.sadhesati_current_status.get("consideration_status")
            if is_undergoing:
                return True
    return False


def render_sadhesati_enhanced(data: KundliData, lang: str = "en") -> str | None:
    if not _has_sadhesati_data(data):
        return None
