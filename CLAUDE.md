...
- **Filler images:** `_fill_gap_pages()` in `pdf_generator.py` overlays promo images on pages that come out mostly blank. Controlled by `filler_images_enabled`, `filler_gap_threshold`, `filler_skip_pages`, `filler_max_images` — and by nothing else: the pass is independent of `pdf_images_enabled` (see below).
- **Image master switch:** `pdf_images_enabled` (`PDF_IMAGES_ENABLED`). When false, `PDFGenerator.generate()` skips the section names in `IMAGE_ONLY_SECTIONS` (`pdf_generator.py`) — every divider/banner page *except* the opening `front_page`. It also sets the Jinja global `show_images`, which the templates use to drop the planet deity PNGs (`graha_profile.html`, `outer_planets.html`). Two images it does **not** govern, despite the name: the promo fillers (gated on `filler_images_enabled` alone, `pdf_generator.py:281`) and `kundli_logo.png` (gated on the separate `show_logo` / `pdf_logo_enabled`, used by `partials/brand_signature.html` — included 5× — and `cover.html`). Charts (the inline-SVG North Indian macro, the cover mandala, and the API chart images) are unaffected. Both globals are set in both Jinja environments — `make_env()` in `sections/__init__.py` and `PDFGenerator.__init__`.
...
- **Credit gating** (`credits_repo.py` + `credits_schema.sql`): generation is metered against `astrologers.credits_balance` — a balance owned by a separate admin/user web app that assigns credits and logs them to `credit_transactions` (`type='assign'`). The sweeper reads the balance **before `claim()`** (so a blocked row costs no claim, no attempt, no API spend, and auto-generates after a top-up) and spends one credit **after a successful Drive upload, before `mark_done`** (the PDF exists, so the credit must not be refunded if `mark_done` fails). Deduction goes through the `consume_kundli_credit` Postgres RPC because PostgREST cannot express `credits_balance = credits_balance - 1` — the function does check + decrement + `type='deduct'` ledger insert in one transaction, so concurrent callers can't double-spend. Note `credit_transactions.total_credits_assigned` is **not** a balance: it's `text`, per-row, and mirrors that row's own `amount`. Unlike the rest of the integration layer this module is **fail-closed** — an unreadable balance blocks generation rather than waving it through. `CREDIT_CHECK_ENABLED=false` restores unmetered generation (rollback, no rebuild — but see the kill-switch note below: it applies on a sweeper recreate, not live).
- **Kill switch:** `SHEET_SWEEPER_ENABLED=false` idles the loop with no code change or rebuild — but `config.py` builds `settings` once at import, so the flag is frozen at process start: restart the sweeper to apply it, and in a container **recreate** it (`up -d --force-recreate sweeper` / Coolify Redeploy), since `docker restart` reuses the old env. Same for `CREDIT_CHECK_ENABLED`. **Observability:** `GET /admin/sheet-jobs` (X-Admin-Key) lists failed/parked rows with `sheet_row` to trace back to the sheet cell. Credit outcomes are log-only: `SHEET NO CREDITS`, `SHEET CREDIT USED`, `SHEET CREDIT NOT DEDUCTED`.
...
- `PDF_IMAGES_ENABLED` — `false` keeps the opening front-page image and drops the dividers, offer banner, and planet deity art. Charts unaffected. It does **not** disable the promo fillers or the logo — those have their own switches below.
- `PDF_LOGO_ENABLED` — `false` drops `kundli_logo.png` from the cover, front matter, TOC, and author's note. Independent of `PDF_IMAGES_ENABLED`.
- `FILLER_IMAGES_ENABLED`, `FILLER_GAP_THRESHOLD`, `FILLER_SKIP_PAGES`, `FILLER_MAX_IMAGES` — tune the post-generation promo overlay pass, independently of `PDF_IMAGES_ENABLED`.
...
- `CREDIT_CHECK_ENABLED` — meters sweeper generation against `astrologers.credits_balance`. Requires `credits_schema.sql` run in Supabase; `false` = unmetered (rollback switch, applied by recreating the sweeper — see the kill-switch note above).
